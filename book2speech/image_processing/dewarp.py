...
def debug_show(name, step, text, display):
    ...
    if DEBUG_OUTPUT != 'file':
        ...
        cv2.putText(image, text, (16, height - 16), cv2.FONT_HERSHEY_SIMPLEX,
...
def pix2norm(shape, pts):
    ...
    offset = np.array([width, height], dtype=pts.dtype).reshape(
        (-1, 1, 2)) * 0.5
...
def norm2pix(shape, pts, as_integer):
    ...
    offset = np.array([0.5 * width, 0.5 * height], dtype=pts.dtype).reshape(
        (-1, 1, 2))
    rval = pts * scl + offset
    if as_integer:
...
def draw_correspondences(img, dstpoints, projpts):
    ...
    for point_a, point_b in zip(projpts, dstpoints):
        cv2.line(display, fltp(point_a), fltp(point_b), (255, 255, 255), 1,
                 cv2.LINE_AA)
...
def get_default_params(corners, ycoords, xcoords):
    ...
    corners_object3d = np.array([[0, 0, 0], [page_width, 0, 0],
    ...
    params = np.hstack((np.array(rvec).flatten(), np.array(tvec).flatten(),
                        np.array(cubic_slopes).flatten(), ycoords.flatten()) +
                       tuple(xcoords))
...
def project_xy(xy_coords, pvec):
    ...
    image_points, _ = cv2.projectPoints(objpoints, pvec[RVEC_IDX],
                                        pvec[TVEC_IDX], K, np.zeros(5))
...
def get_page_extents(small):
    ...
    outline = np.array([[xmin, ymin], [xmin, ymax], [xmax, ymax], [xmax,
                                                                   ymin]])
...
def generate_candidate_edge(cinfo_a, cinfo_b):
    ...
    if (dist > EDGE_MAX_LENGTH or x_overlap > EDGE_MAX_OVERLAP
...
def sample_spans(shape, spans):
    ...
    for span in spans:
        ...
        contour_points = np.array(contour_points, dtype=np.float32).reshape(
            (-1, 1, 2))
...
def visualize_contours(name, small, cinfo_list):
    ...
    for j, cinfo in enumerate(cinfo_list):
        ...
        cv2.circle(display, fltp(cinfo.center), 3, (255, 255, 255), 1,
                   cv2.LINE_AA)
...
def visualize_spans(name, small, pagemask, spans):
    ...
    for i, span in enumerate(spans):
        ...
        cv2.drawContours(regions, contours, -1, CCOLORS[i * 3 % len(CCOLORS)],
                         -1)
...
def remap_image(name, img, small, page_dims, params):
    ...
    page_xy_coords = np.hstack((page_x_coords.flatten().reshape(
        (-1, 1)), page_y_coords.flatten().reshape((-1, 1))))
